...
func (this *Admin) loadRoute() {
	this.AddRoute(func(engine *router.Engine) {
		...
		fmt.Println("route.prefix ", conf.GetString("route.prefix"))
		fmt.Println("route.middleware ", conf.GetString("route.middleware"))

		// 路由

		fmt.Println("route.route ", conf.GetString("route.middleware"))

		admin := router.Groups(engine, conf.GetString("route.prefix"), conf.GetString("route.middleware"))
		//admin := router.Groups(engine, conf.GetString("route.prefix"), "")
		{
			// 常规路由
